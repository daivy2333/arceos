...
> Last updated: 2026-06-19 (M1-T1.1/1.2 完成, T1.3 受 P04 阻塞)  
...
| M1 | RISC-V PLIC 与 UART IRQ 10 基线 | M0 | claim/complete、enable/disable、无 IRQ storm | 📋 change 计划完成，待批准实施 |
| M2 | axtask 单 Future `block_on` 基线 | M0 | Pending 真阻塞、wake 恢复、竞态不丢唤醒 | 📋 change 计划完成，待批准实施 |
...
|----|------|------|-------------|------|
| **M1-T1.3** | uart_irq probe RED 验证 | async-uart-1 | `m1-riscv-plic-baseline` | ⏸️ **Blocked**: 见踩坑 P04，QEMU 16550 offset 8+ 写触发 StoreFault |

## 阻塞(Blocked)

| ID | 主题 | 阻塞原因 | 解阻条件 |
|----|------|----------|----------|
| **M1-T1.3** | Verify RED 见证 | UART 0x1000_0000 + offset ≥8 写触发 StoreFault(踩坑 P04) | 选 A 方案(改 probe 只动 offset 0..7)或选 B(深挖 QEMU/PMP 根因) |
...
<!-- T-010 --> | axtask 最小 Future `block_on` 回移 | P0 | change `m2-axtask-future-block-on` | 仅单 Future executor，不引入 axpoll |
<!-- T-011 --> | Future 调度竞态测试 | P0 | change `m2-axtask-future-block-on` | 14 个任务；含确定性 self-wake 场景 |
<!-- T-012 --> | 创建 `examples/async_uart` 与依赖特性 | P0 | M3 | 本地 uart_16550 path 依赖；multitask + irq + alloc |
<!-- T-013 --> | 移植最小 ArceOS UART adapter/bootstrap | P0 | M3 | Runtime/WakerSet/UartPort/static ring/IRQ trampoline |
<!-- T-014 --> | 异步 RX/TX echo 闭环 | P0 | M3 | 单 UART、单 reader/writer；保留 SBI console |
